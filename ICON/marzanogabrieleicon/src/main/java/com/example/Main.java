...
public class Main {
   ...
   /*       Query q1 = new Query("C:\\Users\\Gabriele\\Desktop\\ICON\\marzanogabrieleicon\\regole.pl')");
        if (q1.hasSolution()) {
            System.out.println("Regole Prolog caricate con successo.");
        }

        String branch = "a";
        Query q2 = new Query("valid_branch('" + branch + "')");
        if (q2.hasSolution()) {
            System.out.println(branch + " è un branch valido.");
        } else {
            System.out.println(branch + " non è un branch valido.");
        } */
